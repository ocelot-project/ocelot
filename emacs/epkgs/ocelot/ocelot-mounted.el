...
(defvar ocelot-mounted-mode-map
  (let ((km (make-sparse-keymap)))
    ...
    ;; searching
    (define-key km "\C-s" 'isearch-forward)
    (define-key km "\C-r" 'isearch-backward)
    (define-key km "\C-\M-s" 'isearch-forward-regexp)
    (define-key km "\C-\M-r" 'isearch-backward-regexp)
    ;; marking
...
(defun ocelot-mounted-display-device (device &optional initial)
  (let* ((inhibit-read-only t)
         ...
         (filesystem (gethash 'filesystem device-info nil))
         (total (gethash 'size device-info nil))
         (label-formatted (cond
                           ((eq status 'updating)
                            (concat label "(updating...)"))
                           ((and (not mount-point)
                                 (equal filesystem "swap"))
                            (concat label "(swap)"))
                           ((and (not mount-point)
                                 (equal filesystem "LVM2_member"))
                            (concat label "(LVM root)"))
                           ((and (not mount-point)
                                 (eq filesystem nil))
                            (concat label "(device root)"))
...
(defun ocelot-mounted-dired ()
  (interactive)
  (let* ((device (gethash
                  (intern (ocelot-mounted-device-at-point))
                  ocelot-mounted-device-info))
         (mount-point (gethash 'mount-point device)))
    (if (stringp mount-point)
        (dired mount-point)
      (user-error "Device at point is not mounted"))))
